...
class GooglePlaywrightBrowserContext:
    ...
    async def login(self):
        ...
        await page.wait_for_timeout(5_000)  # 5 sec. Ждем, пока загрузиться reCAPTCHA
    ...
    async def oauth2(
        ...
        await page.get_by_text("Continue").click(no_wait_after=True)
        ...
        while not oauth_code:
            async with page.expect_event("request") as event_info:
                request: Request = await event_info.value
